...
import { deletePlayer, updatePlayerProfile } from "@/lib/admin-actions";
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ username: string }> }) {
  ...
  const { username } = await params;
  try {
    await deletePlayer(decodeURIComponent(username), admin.id);
    return NextResponse.json({ ok: true });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to delete player." }, { status: 400 });
  }
}
